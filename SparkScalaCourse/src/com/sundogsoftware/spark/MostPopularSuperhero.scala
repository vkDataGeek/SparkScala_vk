...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val flipped = totalFriendsByCharacter.map( x => (x._2, x._1) )
     ...
     // Sort
    val sortedHeros = flipped.sortByKey()
    
    val len= sortedHeros.count
    
    for ( x <- len-10 to len-1) {
     val arrHero = sortedHeros.zipWithIndex.filter(_._2==x).map(_._1).first()
     //   val arrHero = sortedHeros.take(x.toInt).collect()
      val mostPopularName = namesRdd.lookup(arrHero._2)(0)
      println(s"$mostPopularName is the most popular superhero with ${arrHero._1} co-appearances.")
    }
